...
public class Main {
    public static void main(String[] args) {
        try {
            List<City> cities = Files.readAllLines(Paths.get("data/cities.csv"))
                    .stream().map(City::createCity).collect(Collectors.toList());

            List<CustomCity> customCities = cities.stream()
                    .map(CustomCity::createCustomCity).collect(Collectors.toList());

            System.out.println(String.format("Serialized size: %d, externalized size: %d",
                    countAndCheckSerializedBytes(cities),
                    countAndCheckSerializedBytes(customCities)));
    ...
    private static <A,B> Stream <Map.Entry<A,B>>zipStreams(Stream<A> firstStream, Stream<B> secondStream){
        ...
        return secondStream.filter(item -> iterator.hasNext())
                .map(item -> new AbstractMap.SimpleEntry<>(iterator.next(), item));
    }
